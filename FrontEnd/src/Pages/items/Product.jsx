...
import { useNavigate } from "react-router-dom";
import { motion } from "framer-motion";
import axios from "axios"; // ✅ Axios si aan MongoDB ugu xirno

function Product() {
  const navigate = useNavigate();
  const [products, setProducts] = useState([]); // ✅ Xogta MongoDB
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        setProducts(data); // ✅ Kaydi xogta MongoDB
      } catch (error) {
        ...
        {products.map((product, index) => (
          ...
          >
            <img
              ...
              src={product.img} // ✅ Sawirka MongoDB
              alt={product.title}
            />
            <div className="p-4 space-y-2">
              <div className="flex justify-between items-center mb-4">
                <h1 className="text-lg font-semibold text-gray-800">{product.title}</h1>
                <h2 className="text-lg font-bold text-red-500">${product.price}</h2>
              </div>
              <motion.button
                className="w-full bg-[#183b50] text-white text-lg py-2 rounded-lg shadow-md flex justify-center items-center gap-2 transition-all duration-300 hover:bg-[#0f2b3a] hover:scale-105"
                whileTap={{ scale: 0.95 }}
